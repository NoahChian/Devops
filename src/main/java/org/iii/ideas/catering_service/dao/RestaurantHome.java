...
public class RestaurantHome {
	...
	public Restaurant findById(java.lang.Integer id) {
		...
		try {
			Restaurant instance = (Restaurant) sessionFactory
					.getCurrentSession()
					.get("org.iii.ideas.catering_service.dao.Restaurant", id);
			if (instance == null) {
	...
	public List<Restaurant> findByExample(Restaurant instance) {
		...
		try {
			List<Restaurant> results = (List<Restaurant>) sessionFactory
					.getCurrentSession()
					.createCriteria(
							"org.iii.ideas.catering_service.dao.Restaurant")
