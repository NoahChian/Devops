...
<script>
	...
	function uploadOneFile(menuDate, ingredientId, supplierCompanyId, lotNumber, stockDate, filetag, ingredientCategory) {
		...
		if ($("#" + tagname)[0].files[0] != null) {
			...
			runFile.ingredientCategory[0] = ingredientCategory; //食材或調味料
	...
	function show_ingredients(result_content) {
		...
		for (var i = 0; i < result_content.ingredientsCount; i++) {
			...
			for ( var key in result_content.ingredients[i].supplyNameOption) {
				//選項中不再顯示將預設的供應商選項，避免重複
				if (result_content.ingredients[i].supplyName != result_content.ingredients[i].supplyNameOption[key])
					new_table += '<option>' + result_content.ingredients[i].supplyNameOption[key] + '</option>';
			}
			new_table += "</select></td>";
			//製造商名稱
			...
			//產生認證標章的下拉式選單
			new_table += "<tr>";
			new_table += '<td align="left" nowrap="nowrap"><span style="color:red;float:right">*</span>認證標章</td>';
			new_table += '<td><select name="label"  id="label'+ i +'" >';
			new_table += '<option>' + result_content.ingredients[i].label + '</option>'; //預設欄位是廠商輸入的認證標章
			for ( var key in result_content.ingredients[i].labelOption) {
				//選項中不再顯示將預設的認證標章選項，避免重複
			...
			new_table += "</select></td>";
			//認證編號
			new_table += '<td width="20%" align="left" nowrap="nowrap">認證編號</td>';
			...
			/* new_table += '<td align="left" valign="middle"><input  class="max255 numOnly" id="ingredientQuantity'+ i +'" type="text" size="30" value="' + result_content.ingredients[i].ingredientQuantity + '"></td>'; */
			...
			new_table += '<input style="display:none" id="ingredientBatchId'+ i +'" type="text" value="' + result_content.ingredients[i].ingredientBatchId + '">'
					+ '<input id="ingredientId'+ i +'" type="hidden" value="' + result_content.ingredients[i].ingredientId + '">';
	...
	function add_ingredients() {
		...
		new_table += '<option></option>'; //目前新增食材的認證標章選項先寫死只有吉園圃、產銷履歷、有機、CAS、GMP，預設是空值
		...
		new_table += "<tr>";
		new_table += '<td ><span style="color:red;float:right">*</span>進貨日期</td>';
		new_table += '<td ><input type="text" size="30" id="stock_date'+ table_count +'" name="productManufactureDate" class="dateAD dateMode"></td>';
		//20141013_Raymond_增加重量單位
		new_table += '<td width="20%" align="left" nowrap="nowrap"  class="tmpV001">重量</td>';
		...
		/* new_table += '<td align="left" valign="middle"><input  class="max255 numOnly" id="ingredientQuantity'+ table_count +'" type="text" size="30"></td>'; */
		////20140730 Ric  因應更版 更改位置	
		//	new_table += '<td  width="20%" align="left" nowrap="nowrap">批號</td>';
	...
	function update_ingredients() {
		...
		var request_data = {
			"method" : "updateSchoolIngredientDetail",
			"args" : {
				"dishId" : dishId,
				"menuDate" : menuDate,
	...
	function query_ingredient_detail() {
		...
		/// 2.查詢菜色全部食材 ///
		var request_data = {
			"method" : "querySchoolIngredientDetail",
			"args" : {
				"dishId" : dishId,
				"menuDate" : menuDate,
				"schoolId" : schoolId
...
// 	document.onchange = checkSpecialCharacters;
	...
	$(document).ready(function() {
		...
		//檢查是否有抓取到參數
		if (dishId == "get_param_ERROR" || name == "get_param_ERROR" || menuDate == "get_param_ERROR" || schoolId == "get_param_ERROR" || schoolName == "get_param_ERROR") {
			alert("讀取GET參數發生錯誤！");
			return 0;
		}
		...
		var sTitle = menuDate + "-[<b>" + schoolName + "</b>] 單一菜單："
	        ...
	        <div class="section-wrap has-border">
	          ...
	          <div class="form-horizontal kitchen-info">         	
				<table class="table table-bordered table-striped" width="100%">
					<thead>
						<tr>
							<td align="left" nowrap="nowrap" width="50%"><span id="sTitle"></span></td>
							<td id="get_name" align="left" width="50%"></td>
						</tr>
					</thead>
