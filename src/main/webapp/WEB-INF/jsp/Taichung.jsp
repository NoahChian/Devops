...
<script>
	...
	function getSeasoning(mid){
		...
		if(result_content.resStatus == 1){
			for(var i=0; i<result_content.result.length; i++) {
				var html="<tr bgcolor=\"#fffff4\">";
				html+="<td>"+result_content.result[i].ingredientName +"</td>";
				html+="<td>"+result_content.result[i].brand +"</td>";
				html+="<td></td>";
				html+="<td>"+result_content.result[i].source +"</td>";
				html+="</tr>";
				$("#tbSeason").append(html);
		...
		if(response_obj.result == 1)
		{
			...
			if(result_content.resStatus == 1){	//success
				var drop_down_counties = "<select id=\"select_counties\" onChange=\"area()\">";
					...
					drop_down_counties += "<option value=\"27\">臺中市</option><option value=\"17\">臺北市</option>";
			...
			if(result_content.resStatus == 1){	//success
				var drop_down_date = "<select id=\"select_yearMonth\">";
					...
					for(var i=0; i<result_content.date.length; i++) {
						//if(result_content.date[i].yearMonth == yearMonth){	//2013/12
								if(i==0){
	...
	function queryMenu() {
    	...
    	var request_data =	{
				 "method":"customerQueryCateringBySchoolAndDate2",
	 				"args":{
	 					"sid" : sid,
	 					"date" : query_date
	 				}
	...
	//$("#msg").html("查詢中..............");
	 $.get("http://agrapp.coa.gov.tw/GAPWEB/api/label_query/label/14"+qNo,function(data){
		...
		if(!myJsonObj.farmerModel){
			...
			$(label).html("<a  target=\"_blank\" href=\"http://gap.afa.gov.tw/labelresult/?s="+qNo+"\">吉園圃</a>");
		}	
			...
			if(result_content.resStatus == 1){	//success
				...
				*/
				...
				new_div += "<div class=\"SPP_A\"></div>";
						...
						new_div += "<tr bgcolor=\"#fffff4\">";
							new_div += "<td align=\"right\">供應學校</td>";
				...
				//調味料
				var seasoning_div = "<div class=\"TAB_TY_C\"><table class=\"component\" id=\"tbSeason\">";
					seasoning_div += "<tr>";
...
<body>
	<div id="MAIN_TITLE_BAR" class="GRE_ABR h_40px lh_40">
					<div class="TITLE_TXT">菜單查詢(測試頁面)</div>
				</div>
	<div id="dss" class="GRA_ABR">
					<div class="SECOND_TXT"><table class="lh_30" width="100%">
    	<tr>
      		<td colspan="8"></td>
     	</tr>                  
    	...
    	<tr class="h_40px lh_40">
      		<td  colspan="8" align="center" class="BT_IN_BBTER">  
				<div style="font-weight:bold; "><a href="#" onclick="queryMenu()" onkeypress="queryProduct()"/>查詢</a></div>
			</td> 
     	</tr>                  
     	<tr>
      		<td colspan="9"></td>
