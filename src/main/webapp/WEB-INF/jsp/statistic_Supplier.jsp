...
<script src="../../js_ca/statisticSupplier.js"></script>
...
<%
		...
		if(session.getAttribute("account")!=null){
			...
			uNameType = CateringServiceUtil.getSchoolLevel(session.getAttribute("uName").toString());
		}
...
<body>
<!-- 四點設計套版 -->
	<div class="contents-title">供應商數量與清單</div>
...
<%-- 						<% } %> --%>
						<tr>
							...
							<% if ("11".equals(uType)) { %>
								<select id="queryType" onchange='queryKitchen()'>
									<option value ="kitchen">團膳業者 /自設廚房</option>
									<option value ="restaurant">大專美食街</option>
								<select>
							<% } else if(uType.length() == 6) { %>
								<select id="queryType" onchange='queryKitchen()'>
									<option value ="kitchen">團膳業者 /自設廚房</option>
								<select>
							<% } else if("101".equals(uType)) { %>
								<select id="queryType" onchange='queryKitchen()'>
									<option value ="restaurant">大專美食街</option>
								<select>
							<% } else if("SENIOR_HIGH_SCHOOL".equals(uNameType)) { %>
								<select id="queryType" onchange='queryKitchen()'>
									<option value ="kitchen">團膳業者 /自設廚房</option>
									<option value ="restaurant">大專美食街</option>
								<select>
