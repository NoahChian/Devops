...
<script>
	...
	function querySchool() {
		...
		<%Integer county = 0;
			if (session.getAttribute("account") != null) {
				county = AuthenUtil.getCountyNumByUsername(session
		...
		if (response_obj.result == 1) {
			...
			if (result_content.resStatus == 1) { //success
				...
				var new_drop1 = "<select id=\"school_id1\">";
				for (var i = 0; i < result_content.school.length; i++) {
					if (result_content.school[i].sid == "1837478") {
	...
	//產生查詢列表
	function createNullIngredientList() {
		...
		if (response_obj.result == 1) {
			...
			//element.innerHTML = "查詢結果(20筆/頁)  - 查詢菜單筆數：" + response_obj.result_content.nullIngreduentList.length;

			/*
			var total_page = parseInt(response_obj.result_content.getNullIngreduentList.length / row);
			var add = response_obj.result_content.getNullIngreduentList.length % row;
			...
			//組表格
			var head_div = "<table class=\"component\">";
			head_div += "<tr>";
			head_div += "<td colspan=\"14\" class=\"componetContentInput BT_IN_BBTER\">";
			...
			head_div += "</tr>";
			head_div += "<tr>";
			head_div += "<td bgcolor=\"#a1c66a\" class=\"TIT_A\">日期</td>" + "<td bgcolor=\"#a1c66a\" class=\"TIT_A\">學校</td>" + "<td bgcolor=\"#a1c66a\" class=\"TIT_A\">缺少食材菜色</td>";
			head_div += "</tr>";
			...
			if (result_content.resStatus == "1") { //success
				if (result_content.nullIngreduentList.length == 0) {
					...
					for (var i = 0; i < result_content.nullIngreduentList.length; i++) {
						//判斷是否超過上傳時間，超過者不給修改鈕   20140220 KC
						var itemDate = new Date(result_content.nullIngreduentList[i].date + " " + ENV_UPLOAD_DAY_LIMIT);

						if (i % 2 == 0) {
							new_div += "<tr>";
							new_div += "<td bgcolor=\"#edf2e5\" class=\"componetContent\">" + result_content.nullIngreduentList[i].date + "</td>" + "<td bgcolor=\"#edf2e5\" class=\"componetContent\">" + result_content.nullIngreduentList[i].schoolName
									+ "</td>" + "<td bgcolor=\"#edf2e5\" class=\"componetContent\">" + result_content.nullIngreduentList[i].nullIngreduents + "</td>" +
							...
							new_div += "<tr>";
							new_div += "<td bgcolor=\"#FFFFFF\" class=\"componetContent2\">" + result_content.nullIngreduentList[i].date + "</td>" + "<td bgcolor=\"#FFFFFF\" class=\"componetContent2\">" + result_content.nullIngreduentList[i].schoolName
									+ "</td>" + "<td bgcolor=\"#FFFFFF\" class=\"componetContent2\">" + result_content.nullIngreduentList[i].nullIngreduents + "</td>" +
...
<body>
	...
	<div id="query_table" class="h_35px lh_35 GRA_ABR">
		...
		<div class="dis_intb mgl_10 BT_IN_BBTER">
			&nbsp;&nbsp;<input onclick="createNullIngredientList()" id="search" value="查詢" type="button">
		</div>
	</div>
	...
	<div style="color: red">
		<br>！提醒您，於單一食材維護頁面中所編輯的食材，僅顯示於所選擇之學校，其餘學校不受影響。<br>修改後若於食材資料維護頁面進行更新，則此頁之修改會受到覆蓋。
	</div>
