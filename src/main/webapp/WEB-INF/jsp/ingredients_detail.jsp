...
function deleteFile(menuDate, ingredientId, supplierCompanyId, lotNumber, stockDate) {
	...
	function show_oneday_menu(result_content)
	{
		...
		var menu_table = '<div  class="TAB_TY_B"><table class="table table-bordered table-striped" width="100%" ><thead><tr><td colspan="2" align="center"><b>本日所有菜色維護</b></td></tr></thead>';
		...
		{ var bgc ="";
			...
			'<span style="float:right;" ><a class=\"btn btn-primary\" style=\"margin:0\" href="../ingredients_modify/?dishId='+ result_content.dishs[i].dishId +'&name=' + result_content.dishs[i].name +'&menuDate='+ menu_date +'">食材維護</a></span></td>'+
			"</tr>";
		}
	...
	function query_oneday_menu()
	{
		/// 1.查詢菜單內容 ///
		var request_data =	{
								"method":"queryIngredientListByDate",
								"args":{
									"menuDate":menu_date,
								}
							};
	...
	function update_seasonings()
	{
		...
		for(var i = 0 ; i< stable_count; i++)
		{
			...
			if(stable_status[i] == 1)
			{
				...
				if(seasoning.stockDate == "")
				{
					alert("請輸入 "+ seasoning.name +" 進貨日期!")
					return 0;
				}
		...
		var request_data =	{
								"method":"updateSeasoningDetailv2",
								"args":{
										"menuDate" : menu_date,		
	...
	function add_seasoning()
	{
		...
		var seasoning_table =  '<table class="" width="100%" id="s_table'+ stable_count +'">';
		...
		seasoning_table += "<tr>"+
								...
								'<option></option>'+//目前新增食材的驗證標章選項先寫死只有吉園圃、v、有機、CAS、GMP，預設為空
								'<option>CAS</option>'+	
		...
		seasoning_table	+=	'<tr>'+
								'<td   width="20%" align="left" nowrap="nowrap"><span style="color:red;float:right">*</span>進貨日期</td>'+
								'<td   align="left" valign="absmiddle" >'+
	...
	function show_seasoning(result_content)
	{
		...
		for(var i = 0 ; i<result_content.seasoningsCount; i++)
		{
			...
			for(var key in result_content.seasonings[i].supplierOption)
			{
				//選項中不再顯示將預設的供應商選項，避免重複
				if(result_content.seasonings[i].supplyName != result_content.seasonings[i].supplierOption[key].supplierName)
					seasoning_table +='<option>'+ result_content.seasonings[i].supplierOption[key].supplierName +'</option>';
			}
			seasoning_table +=		'</select></td>'+
									 '<td  width="20%" align="left" nowrap="nowrap" class="tmpV001">製造商名稱</td>'+
					 				 '<td  align="left" valign="middle">'+
					 	 		 	////20140730 Ric  因應更版 改為顯示
					 	 		 	//'<input  id="s_brand'+ stable_count +'" type="hidden" value="' + result_content.seasonings[i].brand + '">'+
	 ...
	 */
	 function get_param(arg_str){
		...
		/*
		$(function() {
			$( "#stock_date" ).datepicker({ dateFormat: "yy/mm/dd" });		//調味料進貨日期
			$( "#product_date" ).datepicker({ dateFormat: "yy/mm/dd" });	//調味料生產日期
			$( "#valid_date" ).datepicker({ dateFormat: "yy/mm/dd" });		//調味料有效日期
		});
	*/
		/*
...
// 		document.onchange = checkSpecialCharacters;
		...
		$(document).ready(function(){	
			...
			//檢查是否有抓取到參數
			if(menu_date== "get_param_ERROR" || menu_date == null)
			{
				alert ("讀取GET參數發生錯誤！");
				return 0;
			}
		...
		<div class="contents-wrap">
			...
			<div id="seasoning_modify" class="TAB_TY_D">
				...
				<table class="table table-bordered table-striped" width="100%" >
					<thead>
						<tr>
							<td colspan="2" align="center"><b>本日所有調味料維護</b></td>
						</tr>
					</thead>
					<tr>
	...
	<div style="display:none">
						...
						<td colspan="2">
							...
							<table width="100%">
								<tr>
									...
									<input type="text" id="sourceName_{0}" name="sourceName_{0}" size="50" value="沙拉油">
									<span></span>
									<input type="hidden" id="sourceId_{0}" name="sourceId_{0}" class="sourceSelectNone">
									<span id="productInfo_{0}" style="vertical-align:middle"></span>
									</td>
								</tr>
								<tr>
									<td align="left" nowrap="nowrap"><span style="color:red;float:right">*</span>供應商名稱</td>
						  ...
						  <select>
							   ...
							   <option value='HACCP'>HACCP</option>
								...
								<tr>	
									<td>有效日期</td>				
									<td>
									<input type="text" id="valid_date" name="productManufactureDate" class="dateAD" value='2014/12/31'>
									<span></span>
									</td>
								</tr>
								<tr>	
		...
		<script type="text/javascript"><!-- Justin: Add this for rendering on Firefox -->
